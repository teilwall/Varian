...
def create_appointment(request , user):
	...
	if request.method == "POST":
		...
		new_appointment = Appointment(doctorid = doctor , patientid = patient ,time = request.POST['time'] ,  date = request.POST['date'] , status  = status )
...
def docter_appointment(request):
	...
	if request.user:
		status = request.user
	user_id = User.objects.get(username=request.user)
	docter= Doctor.objects.get(username=user_id)
